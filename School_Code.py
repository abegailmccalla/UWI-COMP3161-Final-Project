...
with open('School_Database.sql', 'w') as file:
    ...
    file.write("\n-- Assignment --\n")
    file.write("CREATE TABLE Assignment (Assign_Id INT PRIMARY KEY, Course_Id VARCHAR(255), Assign_Title VARCHAR(255), FOREIGN KEY (Course_Id) REFERENCES Course(Course_Id));\n")
    ...
    for c in courses:
        ...
        for c1 in range(0, 3):
            ...
            file.write(f"INSERT INTO Assignment (Assign_Id, Course_Id, Assign_Title) VALUES ({a_id}, '{c_id}', '{a_title}');\n")
    ...
    for c in range(0, 200):
        for s in range(i, 1000, 5): #100000, 500
            stds = [st[0] for st in students[s:s + 5]] #500
    ...
    for e in enrols:
        ...
        for cse in e[1]:
            ...
            avgG = int(sum(stu_cse_grades)/3)
            file.write(f"INSERT INTO Enrols (Std_Id, Course_Id, Grade) VALUES ({st_id}, '{cse}', {avgG});\n")
    
    # Create Discussion_Forum Table
    file.write("\n-- Discussion_Forum --\n")
    file.write("CREATE TABLE Discussion_Forum (Forum_Id INT PRIMARY KEY, Course_Id VARCHAR(255), Forum_Title VARCHAR(255), FOREIGN KEY (Course_Id) REFERENCES Course(Course_Id));\n")
    # Generate fake data for Discussion_Forum
    forum_id = 1
    course_ids = [cids[0] for cids in courses]
    for cid in course_ids:
        title = f"{fake.bs().capitalize()} Forum"
        file.write(f"INSERT INTO Discussion_Forum (Forum_Id, Course_Id, Forum_Title) VALUES ({forum_id}, '{cid}', '{title}');\n")
        course_forum_map[cid] = forum_id
        forum_id += 1
    
    # Create Discussion_Thread Table
    file.write("\n-- Discussion_Thread --\n")
    file.write("CREATE TABLE Discussion_Thread (Thread_Id INT PRIMARY KEY, Forum_Id INT, date Date, FOREIGN KEY (Forum_Id) REFERENCES Discussion_Forum(Forum_Id));\n")
    # Create Generates Table
    file.write("\n-- Generates --\n")
    file.write("CREATE TABLE Generates (Std_Id INT, Thread_Id INT PRIMARY KEY, FOREIGN KEY (Std_Id) REFERENCES Student(Std_Id), FOREIGN KEY (Thread_Id) REFERENCES Discussion_Thread(Thread_Id));\n") 
    # Generate fake data for Discussion_Thread and Generates
    poss_stu = []
    thread_id = 1
    for cid, fid in course_forum_map.items():
        ...
        for _ in range(num_threads):
            ...
            file.write(f"INSERT INTO Discussion_Thread (Thread_Id, Forum_Id, date) VALUES ({thread_id}, {fid}, '{date}');\n")
            threads.append((thread_id, fid, date))
            file.write(f"INSERT INTO Generates (Std_Id, Thread_Id) VALUES ({sid}, {thread_id});\n")
            generates.append((sid, thread_id))
            thread_id += 1
    ...
    # Create Topic Table
    file.write("\n-- Topic --\n")
    file.write("CREATE TABLE Topic (Topic_Id INT PRIMARY KEY, Course_Id VARCHAR(255), Topic_Title VARCHAR(255), FOREIGN KEY (Course_Id) REFERENCES Course(Course_Id));\n")
    ...
    for cid in course_ids:
        ...
        for _ in range(num_topics):
            ...
            file.write(f"INSERT INTO Topic (Topic_Id, Course_Id, Topic_Title) VALUES ({topic_id}, '{cid}', '{title}');\n")
    ...
    file.write("\n-- Content --\n")
    file.write("CREATE TABLE Content (Content_Id INT PRIMARY KEY, Topic_Id INT, link VARCHAR(255), file VARCHAR(255), slide VARCHAR(255), FOREIGN KEY (Topic_Id) REFERENCES Topic(Topic_Id));\n")
    ...
    for topic_list in course_topic_map.values():
        for tid in topic_list:
            ...
            for _ in range(num_content):
                ...
                file.write(f"INSERT INTO Content (Content_Id, Topic_Id, link, file, slide) VALUES ({content_id}, {tid}, '{link}', '{fle}', '{slide}');\n")
                contents.append((content_id, tid, link, fle, slide))
                content_id += 1

    # Create Events Table
    file.write("\n-- Events --\n")
    file.write("CREATE TABLE Events (Event_Id INT PRIMARY KEY, Course_Id VARCHAR(255), Start_Date DATE, End_Date DATE, Event_Title VARCHAR(255), FOREIGN KEY (Course_Id) REFERENCES Course(Course_Id));\n")
    ...
    for cid in course_ids:
        ...
        for _ in range(num_events):
            ...
            title = fake.catch_phrase().replace("'", "''")
            file.write(f"INSERT INTO Events (Event_Id, Course_Id, Start_Date, End_Date, Event_Title) VALUES ({event_id}, '{cid}', '{start_date}', '{end_date}', '{title}');\n")
